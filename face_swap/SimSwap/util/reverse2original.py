...
def postprocess(swapped_face, target, target_mask,smooth_mask):
    ...
    mask_tensor = torch.from_numpy(target_mask.copy().transpose((2, 0, 1))).float().mul_(1/255.0).cpu()
...
def reverse2wholeimage(b_align_crop_tenor_list, swaped_imgs, mats, crop_size, oriimg, parsing_model, norm, save_path="./images/output.jpg", use_mask=False):
