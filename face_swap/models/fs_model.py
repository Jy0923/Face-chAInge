...
class fsModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        # Id network
        self.netArc = torch.load(opt.Arc_path)['model'].module.to(device)
        self.netArc.eval()
    ...
    def _gradinet_penalty_D(self, netD, img_att, img_fake):
        # interpolate sample
        bs = img_fake.shape[0]
        alpha = torch.rand(bs, 1, 1, 1).expand_as(img_fake).cpu()
        ...
        grad = torch.autograd.grad(outputs=pred_interpolated,
                                   inputs=interpolated,
                                   grad_outputs=torch.ones(pred_interpolated.size()).cpu(),
                                   retain_graph=True,
                                   create_graph=True,
                                   only_inputs=True)[0]
