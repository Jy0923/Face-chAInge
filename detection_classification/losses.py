    ...
    def forward(self,classifications,bbox_regressions,ldm_regressions,anchors,annotations):
        ...
        for j in range(batch_size):
            ...
            if bbox_annotation.shape[0] == 0:
                bbox_regression_losses.append(torch.tensor(0.,requires_grad=True).cpu())
                classification_losses.append(torch.tensor(0.,requires_grad=True).cpu())
                ldm_regression_losses.append(torch.tensor(0.,requires_grad=True).cpu())
            ...
            if focal_loss:
                alpha = 0.25
                gamma = 2.0            
                alpha_factor = torch.ones(targets.shape).cpu() * alpha
                ...
                cls_loss = torch.where(torch.ne(targets, -1.0), cls_loss, torch.zeros(cls_loss.shape).cpu())
            ...
            if positive_indices.sum() > 0:
                ...
                # Rescale
                bbox_targets = bbox_targets/torch.Tensor([[0.1, 0.1, 0.2, 0.2]]).cpu()
            ...
            if ldm_positive_indices.sum() > 0 :
                ...
                ldm_targets = ldm_targets/scale.cpu()
